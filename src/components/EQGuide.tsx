...
export const EQGuide: React.FC = () => {
  ...
  const filteredInstrumentAdvice = showGeneralView ? [] : EQ_INSTRUMENT_ADVICE.filter(advice => {
    ...
    const matchesZone = selectedZone === 'all' || advice.frequencyRange.toLowerCase().includes(
      FREQUENCY_ZONES.find(z => z.id === selectedZone)?.label.split(' ')[0].toLowerCase() || ''
    );
    return matchesInstrument && matchesZone;
  });

  // Filter general guidance by zone if needed
  const filteredGeneralGuidance = selectedZone === 'all' 
    ? generalEQGuidance 
    : generalEQGuidance.filter(guidance => guidance.zoneId === selectedZone);
      ...
      <div className="flex flex-wrap gap-6 mb-6 items-end">
        <div className="flex-1 min-w-[180px]">
          ...
          <select
            ...
            className="w-full px-3 py-2 bg-gray-700 border border-gray-600 rounded-md text-gray-100 focus:border-purple-500 focus:ring-1 focus:ring-purple-500"
          >
                ...
                <div className="flex flex-col gap-3">
                  ...
                  <div className="flex items-center gap-2">
                    <span className={`px-2 py-1 text-xs font-medium rounded ${
                      guidance.action.toLowerCase().includes('boost') || guidance.action.toLowerCase().includes('enhance') 
                        ? 'bg-green-600 text-white' 
                        : guidance.action.toLowerCase().includes('cut') || guidance.action.toLowerCase().includes('filter') 
          ...
          <div className="col-span-full">
            <Card className="bg-gray-700/80 text-center py-16">
              <p className="text-gray-300 text-lg">
                No EQ advice matches your current filters.<br />
                Try another instrument or frequency zone.
              </p>
            </Card>
          </div>
        ) : (
            ...
            return (
              <Card key={index} className="bg-gray-800/80 h-fit">
                <div className="flex flex-col gap-3">
                  ...
                  <div className="flex items-center gap-2">
                    <span className={`px-2 py-1 text-xs font-medium rounded ${
                      advice.action.toLowerCase().includes('boost') 
                        ? 'bg-green-600 text-white' 
                        : advice.action.toLowerCase().includes('cut') 
                        ? 'bg-red-600 text-white' 
