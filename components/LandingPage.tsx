      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid md:grid-cols-3 gap-8">
          {[ 
            ...
            <div key={num} className="text-center space-y-6">
    ...
    <section className="px-6 py-20 bg-black/20">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid md:grid-cols-3 gap-8">
          {[
            {
              title: 'Flexible pricing tiers for individual producers',
              desc: 'We offer customizable pricing plans tailored to meet various producer needs and budgets.'
            },
            {
              title: 'School licensing options for educational institutions',
              desc: 'Educational institutions can access special licensing for students and educators to enhance learning.'
            },
            {
              title: 'Exclusive features to enhance user experience',
              desc: 'Unique features are designed to optimize workflow and provide valuable support throughout the production process.'
            }
          ].map(({ title, desc }) => (
          ...
          {[
            {
              ...
              desc: 'Recurring revenue from premium features, advanced AI models, and unlimited generations.',
              metric: '85% Gross Margin'
            },
            {
              title: 'Marketplace',
              desc: 'Commission from sample packs, presets, and templates created by our community.',
              metric: '30% Commission'
            },
            {
              title: 'Enterprise',
              desc: 'Custom solutions for record labels, music schools, and production companies.',
              metric: '$50K+ ARR'
            }
          ].map(({ title, desc, metric }) => (
            <div key={title} className="bg-gray-800/30 rounded-lg p-8 border border-orange-500/20">
              <h3 className="text-2xl font-bold text-white mb-4">{title}</h3>
              <p className="text-gray-300 mb-6">{desc}</p>
              <div className="text-orange-500 font-bold text-lg">{metric}</div>
            </div>
          ))}
        </div>
      </div>
